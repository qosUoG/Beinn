...
export class Workspace {
    ...
    async save() {
        const { step, completed, } = await beginProcedure("SAVE WORKSPACE")
    ...
    connect = async (path: string) => {
        ...
        this._path = path
        const { step, completed, cancelled, appendLog } = await beginProcedure("CONNECT MEALL")
        await tick()
        ...
        const cont = await step("Setup uv in workspace",
            async () => {
                ...
                // Run uv init 
                await shell({ fn: "uv", cmd: "init", cwd: this._path, appendLog })
                return true
            }
        ...
        await step("Copy newest version of meall to workspace",
            async () => {
                await shell({ fn: "uvx", cmd: "copier copy git+https://github.com/qosUoG/Beinn.git ./meall -f", cwd: path, appendLog })
            }
        )
        ...
        await step("Install required dependencies",
            async () => {
                await shell({ fn: "uv", cmd: "add git+https://github.com/qosUoG/Beinn#subdirectory=packages/cnoc --branch main", cwd: path, appendLog })

                await shell({ fn: "uv", cmd: "sync", cwd: path, appendLog })
                await shell({ fn: "uv", cmd: "add fastapi", cwd: path, appendLog })
                await shell({ fn: "uv", cmd: "add fastapi[standard]", cwd: path, appendLog })
                await shell({ fn: "uv", cmd: "add aiosqlite", cwd: path, appendLog })

                // In case cnoc is already installed and stale
                await shell({ fn: "uv", cmd: "lock --upgrade-package cnoc", cwd: path, appendLog })
            }
        )
        ...
        await step("Wait until meall is online",
            async () => {
                ...
                this.meall_pid = (await meallGetPid_throwable()).pid
            }
        )
