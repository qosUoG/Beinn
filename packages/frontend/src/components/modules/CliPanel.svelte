<script lang="ts">
	...
	let display_beinn = $state(false);
	let display_python = $state(false);
	...
	let displaying_logs = $derived.by(() => {
		return gstore.logs.logs
			.filter((log) => {
				if (log.source === "beinn" && display_beinn) return true;
				if (log.source === "python" && display_python) return true;
				...
				<div class="frow-1 items-start">
					...
					<div
						class={cn(
							...
							source === "beinn" ? "text-yellow-300" : "",
							source === "python" ? "text-cyan-300" : "",
